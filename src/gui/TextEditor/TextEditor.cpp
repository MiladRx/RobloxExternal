...
std::string TextEditor::GetText(const Coordinates & aStart, const Coordinates & aEnd) const
{
	...
	auto line = aStart.mLine;
	auto lineEnd = aEnd.mLine;
	auto i = GetCharacterIndex(aStart);
	auto iEnd = GetCharacterIndex(aEnd);
	size_t approx = 0;

	for (size_t n = line; n < lineEnd; n++)
		approx += mLines[n].size();

	out.reserve(approx + approx / 8);

	while (i < iEnd || line < lineEnd)
	{
		if (line >= (int)mLines.size())
			break;

		auto& row = mLines[line];
		if (i < (int)row.size())
		{
			out += row[i].mChar;
			i++;
...
void TextEditor::DeleteRange(const Coordinates & aStart, const Coordinates & aEnd)
{
	...
	{
		auto& first = mLines[aStart.mLine];
		auto& last = mLines[aEnd.mLine];

		first.erase(first.begin() + start, first.end());
		last.erase(last.begin(), last.begin() + end);
