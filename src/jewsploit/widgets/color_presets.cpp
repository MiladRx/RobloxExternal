...
namespace
{
	...
	int nearest_preset(const float col[4])
	{
		int best = -1;
		float bd = 0.02f;
		for (int i = 0; i < g_preset_n; i++)
		{
			float d = col_dist(col, g_presets[i]);
			if (d < bd)
			{
				bd = d;
				best = i;
...
bool ng::color_presets(const char* id, const char* title, const char* desc, float col[4], bool shown)
{
	if (!id || !title || !col)
	{
		return false;
	...
	bool brush_hit = ImGui::IsItemClicked() && e > 0.55f;
	...
	float dots_x0 = brush_x + brush_sz + 10.f;
