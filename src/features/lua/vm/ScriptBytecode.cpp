...
{
	std::ostringstream ss;
	ss << "-- decompiled with MiladExternal\n";
...
bool ReadRaw(std::uint64_t script_addr, const std::string& class_name, std::vector<std::uint8_t>& out)
{
	...
	const std::uint64_t ptr = g_Memory.Read<std::uint64_t>(bc_obj + Offsets::ByteCode::Pointer);
	const std::uint64_t size = g_Memory.Read<std::uint64_t>(bc_obj + Offsets::ByteCode::Size);
