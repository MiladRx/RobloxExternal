	...
	namespace Features {
		...
		bool Fog::Apply(std::uint64_t lighting)
		{
			...
			if (w.fog)
			{
				...
				dirty |= LightingInvalidate::write_if_changed<float>(lighting + Offsets::Lighting::FogStart, start);
				dirty |= LightingInvalidate::write_if_changed<float>(lighting + Offsets::Lighting::FogEnd, end);
				dirty |= LightingInvalidate::write_if_changed<Color3>(lighting + Offsets::Lighting::FogColor,
