...
// empty cursor, otherwise the system one blinks
...
{
    // keep the counter negative, Roblox increments it
    for (int i = 0; i < 8; ++i)
    {
...
DWORD WINAPI CursorSuppressThread(LPVOID)
{
    ...
    while (!g_thread_stop.load(std::memory_order_acquire))
    {
        ...
        // the overlay also sometimes blinks during click-through
        HWND overlay = Renderer::GetHwnd();
...
void Render()
{
    ...
    // hide immediately, don't wait for a thread tick
