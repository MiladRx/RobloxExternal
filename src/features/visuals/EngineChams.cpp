...
{
	return g_Memory.IsWritable(layer + Offsets::MaterialLayer::FillModeByte, 1) &&
	       g_Memory.IsWritable(layer + Offsets::MaterialLayer::ColorData, sizeof(std::uint32_t));
}
...
void RestoreLayersLocked(uintptr_t ent)
{
	...
	for (uintptr_t layer : lit->second)
	{
		...
		if (alive && LayerAlive(layer))
		{
			...
			g_Memory.Write<std::uint8_t>(layer + Offsets::MaterialLayer::FillModeByte, b.fillmode);
			if (g_Memory.IsWritable(layer + Offsets::MaterialLayer::MatFlags, 4))
			{
				g_Memory.Write<std::uint32_t>(layer + Offsets::MaterialLayer::MatFlags, b.matflags);
			}
			if (g_Memory.IsWritable(layer + Offsets::MaterialLayer::Param, 4))
			{
				g_Memory.Write<std::uint32_t>(layer + Offsets::MaterialLayer::Param, b.param);
			}
			if (g_Memory.IsWritable(layer + Offsets::MaterialLayer::Flags2, 4))
			{
				g_Memory.Write<std::uint32_t>(layer + Offsets::MaterialLayer::Flags2, b.flags2);
			}
			g_Memory.Write<std::uint32_t>(layer + Offsets::MaterialLayer::ColorData, b.color);
		}
...
bool PushPartPos(const std::shared_ptr<Instance>& part, std::vector<Vector3>& out)
{
	...
	Vector3 p = g_Memory.Read<Vector3>(prim + Offsets::Primitive::Position);
...
void ApplyLayers(uintptr_t ent, std::uint8_t fill, std::uint32_t param, std::uint32_t flags2, std::uint32_t color)
{
	...
	uintptr_t arr = g_Memory.Read<uintptr_t>(ent + Offsets::FastClusterEntity::TechniqueArrayPtr);
	if (!g_Memory.IsValid(arr))
	{
		return;
	}

	uintptr_t begin = g_Memory.Read<uintptr_t>(arr + Offsets::TechniqueArray::BeginOffset);
	uintptr_t end = g_Memory.Read<uintptr_t>(arr + Offsets::TechniqueArray::EndOffset);
	...
	for (std::size_t i = 0; i < count; ++i)
	{
...

		{
			...
			{
				LayerBackup b{};
				b.fillmode = g_Memory.Read<std::uint8_t>(layer + Offsets::MaterialLayer::FillModeByte);
				b.matflags = g_Memory.Read<std::uint32_t>(layer + Offsets::MaterialLayer::MatFlags);
				b.param = g_Memory.Read<std::uint32_t>(layer + Offsets::MaterialLayer::Param);
				b.flags2 = g_Memory.Read<std::uint32_t>(layer + Offsets::MaterialLayer::Flags2);
				b.color = g_Memory.Read<std::uint32_t>(layer + Offsets::MaterialLayer::ColorData);
				g_layers[layer] = b;
				g_ent_layers[ent].push_back(layer);
			}
		}

		g_Memory.Write<std::uint8_t>(layer + Offsets::MaterialLayer::FillModeByte, fill);
		if (g_Memory.IsWritable(layer + Offsets::MaterialLayer::MatFlags, 4))
		{
			g_Memory.Write<std::uint32_t>(layer + Offsets::MaterialLayer::MatFlags, 0);
		}
		if (g_Memory.IsWritable(layer + Offsets::MaterialLayer::Param, 4))
		{
			g_Memory.Write<std::uint32_t>(layer + Offsets::MaterialLayer::Param, param);
		}
		if (g_Memory.IsWritable(layer + Offsets::MaterialLayer::Flags2, 4))
		{
			g_Memory.Write<std::uint32_t>(layer + Offsets::MaterialLayer::Flags2, flags2);
		}
		g_Memory.Write<std::uint32_t>(layer + Offsets::MaterialLayer::ColorData, color);
	}
}
