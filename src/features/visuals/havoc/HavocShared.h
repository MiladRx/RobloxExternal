...
inline std::unordered_map<std::string, LootKind> g_catalog;
inline std::chrono::steady_clock::time_point g_catalog_next{};

inline bool WorldToScreen(const Matrix4x4& matrix, const Vector2& dimensions,
...
inline std::shared_ptr<Instance> ResolvePath(Instance root, const char* const* parts, int count)
{
...
inline bool ContainsI(const std::string& hay, const char* needle)
{
	return ToLower(hay).find(ToLower(needle)) != std::string::npos;
}

inline LootKind FolderToKind(const std::string& folder)
{
...
inline void IngestCatalogFolder(const Instance& folder, LootKind kind)
{
...
inline bool HasChildNamed(const Instance& node, const char* name)
{
...
inline bool LootKindEnabled(LootKind kind)
{
	const auto& f = g_Settings.esp.loot_filter;
	if (kind == LootKind::Weapon) return f[Settings::LOOT_WEAPONS];
	if (kind == LootKind::Mag) return f[Settings::LOOT_MAGS];
	if (kind == LootKind::Ammo) return f[Settings::LOOT_AMMO];
	if (kind == LootKind::Attachment) return f[Settings::LOOT_ATTACHMENTS];
	if (kind == LootKind::Medical) return f[Settings::LOOT_MEDICAL];
	if (kind == LootKind::Valuable) return f[Settings::LOOT_VALUABLES];
	if (kind == LootKind::Tool) return f[Settings::LOOT_TOOLS];
	if (kind == LootKind::Electronics) return f[Settings::LOOT_ELECTRONICS];
	if (kind == LootKind::Household) return f[Settings::LOOT_HOUSEHOLDS];
	if (kind == LootKind::Document) return f[Settings::LOOT_DOCUMENTS];
	if (kind == LootKind::Building) return f[Settings::LOOT_OTHER];
	if (kind == LootKind::Gear) return f[Settings::LOOT_OTHER];
	if (kind == LootKind::Other) return f[Settings::LOOT_OTHER];
	return false;
}

inline void CollectFolderModels(const Instance& folder, std::vector<CachedItem>& out, bool classify)
{
	if (!g_Memory.IsValid(folder.address)) return;
	for (const auto& child : folder.GetChildren()) {
		if (!g_Memory.IsValid(child.address)) continue;
...
inline void RefreshCache(FolderCache& cache, const std::vector<std::shared_ptr<Instance>>& folders,
...
inline bool ProjectObb(const BasePart& part, const Matrix4x4& view, const Vector2& viewport,
	float scale_x, float scale_y,
...
inline void DrawShaderObb(ImDrawList* draw_list, const std::vector<ImVec2>& hull,
...
inline void DrawWorldEntries(ImDrawList* draw_list, ImFont* font, float font_size,
