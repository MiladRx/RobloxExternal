...
#endif
#include "features/aim/BoundSilent.h"
...
constexpr std::uintptr_t desc_rva = Offsets::WorldRoot::RaycastBoundDesc;
constexpr std::uintptr_t bound_fn = Offsets::WorldRoot::RaycastBoundFn;
...
#pragma pack(pop)
	...
	{
		const std::uint64_t ws = Cheat::Globals::Workspace->address;
		if (g_Memory.IsValid(ws))
		{
			const std::uint64_t cam = g_Memory.Read<std::uint64_t>(ws + Offsets::Workspace::CurrentCamera);
			if (g_Memory.IsValid(cam))
			{
				const Vector3 cp = g_Memory.Read<Vector3>(cam + Offsets::Camera::Position);
				p.cam_x = cp.x;
				p.cam_y = cp.y;
				p.cam_z = cp.z;
			}
