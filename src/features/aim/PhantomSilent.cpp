...
std::uint64_t PartPrimitive(std::uint64_t part)
{
    if (!g_Memory.IsValid(part))
        return 0;
    return g_Memory.Read<std::uint64_t>(part + Offsets::BasePart::Primitive);
}
...
void SetRotation(std::uint64_t part_or_cam, const Matrix3x3& m, bool is_part)
{
    ...
    if (is_part) {
        ...
        if (!g_Memory.IsValid(prim))
            return;
        g_Memory.WriteRaw(prim + Offsets::Primitive::Rotation, m.data, sizeof(m.data));
    } else {
        g_Memory.WriteRaw(part_or_cam + Offsets::Camera::Rotation, m.data, sizeof(m.data));
    }
}
...
void run(const Vector3& targetpos)
{
    ...
    const std::uint64_t ve = g_Memory.Read<std::uint64_t>(s_base + Offsets::VisualEngine::Pointer);
    if (!g_Memory.IsValid(ve))
        return;

    if (!ghascachedmatrix) {
        g_Memory.ReadRaw(ve + Offsets::VisualEngine::ViewMatrix, gcachedviewmatrix, sizeof(gcachedviewmatrix));
    ...
    Vector3 pos = camera_is_part
        ? GetPartPos(camera)
        : g_Memory.Read<Vector3>(camera + Offsets::Camera::Position);
