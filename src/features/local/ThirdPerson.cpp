...
std::uint64_t find_local_hum()
{
	...
	auto chara = g_Memory.Read<std::uint64_t>(lp + Offsets::Player::ModelInstance);
...
void poke_offset(std::uint64_t hum, const Vector3& off)
{
	if (!g_Memory.IsValid(hum))
		return;
	g_Memory.Write<Vector3>(hum + Offsets::Humanoid::CameraOffset, off);
}
...
void Cheat::Features::ThirdPerson::Tick()
{
	...
	if (!g_on || g_hum != hum)
	{
		g_old_off = g_Memory.Read<Vector3>(hum + Offsets::Humanoid::CameraOffset);
		g_got_old = true;
		g_hum = hum;
		g_on = true;
	}
