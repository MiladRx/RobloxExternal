...
void CollectJobs(const PlayerCache& c, std::vector<PartJob>& out)
{
	...
	switch (hb.part)
	{
	case Settings::HB_HEAD:
		add(c.head);
		break;
	case Settings::HB_HRP:
		add(c.humanoidRootPart);
		break;
	case Settings::HB_TORSO:
		add(c.upperTorso);
		add(c.lowerTorso);
		break;
	case Settings::HB_ARMS:
		add(c.leftUpperArm);
		add(c.leftLowerArm);
		add(c.leftHand);
		add(c.rightUpperArm);
...
void Render()
{
	...
	uintptr_t ve = g_Memory.Read<uintptr_t>(s_mod + Offsets::VisualEngine::Pointer);
