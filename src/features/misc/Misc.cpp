...
void freecam_tick(float dt)
{
	...
	if (!want || !g_Memory.IsValid(hum))
	{
		...
		{
			g_Memory.Write<Vector3>(hum + Offsets::Humanoid::CameraOffset, s_saved_off);
			g_Memory.Write<bool>(hum + Offsets::Humanoid::PlatformStand, s_saved_plat);
			g_Memory.Write<bool>(hum + Offsets::Humanoid::AutoRotate, s_saved_autorot);
		}
		s_on = false;
		return;
	}
	...
	{
		s_on = true;
		s_saved_off = g_Memory.Read<Vector3>(hum + Offsets::Humanoid::CameraOffset);
		s_saved_plat = g_Memory.Read<bool>(hum + Offsets::Humanoid::PlatformStand);
		s_saved_autorot = g_Memory.Read<bool>(hum + Offsets::Humanoid::AutoRotate);
...
void Cheat::Features::Misc::Tick(float dt)
{
	...
	{ /* fps unlock */
		...
		if (s_sched)
		{
			std::uint64_t addr = s_sched + Offsets::TaskScheduler::MaxFPS;
			if (m.fps_unlock)
			{
				if (!s_got_orig)
				{
	...
	{ /* fov */
		...
		if (m.fov && Cheat::Globals::Workspace)
		{
			...
			if (cam)
			{
				std::uint64_t addr = cam->address + Offsets::Camera::FieldOfView;
